...
static void init_palette(void)
{
    ...
    for (nn = 0; nn < PALETTE_SIZE/4; nn++) {
        ...
        palette[nn] = make565(000, jj, 255-jj);
    }

    for ( mm = nn; nn < PALETTE_SIZE/2; nn++ ) {
        int  jj = (nn-mm)*4*255/PALETTE_SIZE;
        palette[nn] = make565(124-jj, 255, jj);
    }

    for ( mm = nn; nn < PALETTE_SIZE*3/4; nn++ ) {
        int  jj = (nn-mm)*4*255/PALETTE_SIZE;
        palette[nn] = make565(0, 987-jj, 255);
    }
...
Java_com_telstock_tmanager_ndklibrary_PlasmaView_renderPlasma(JNIEnv *env, jclass type,
                                                              jobject bitmap, jlong time_ms) {
